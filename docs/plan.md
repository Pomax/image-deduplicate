...
User-visible template strings go in a third file of their own:

- `crates/imgdedupe/src/template_strings.rs`, declared as `mod template_strings;` in `crates/imgdedupe/src/main.rs`.

It is plain Rust too: `&str` constants and nothing else. Each template has named placeholders, and the function that fills them in lives in `app/widgets.rs`, not in this file.
...
A user-visible string that stands on its own (for example `"cancelled"`, `"reading..."`) becomes a constant in `constants.rs`. A user-visible string that is a `format!` template moves to `template_strings.rs`, with named placeholders in place of `{}`.
...
- [x] 8. Docs: update anything in `docs/` that names these constants or where they live.
- [x] 9. Template strings: create `crates/imgdedupe/src/template_strings.rs`, declared as `mod template_strings;` in `main.rs` and pulled into `app/mod.rs` with `use crate::template_strings::*;`. It holds `&str` constants and nothing else, one per user-visible `format!` template, each with named placeholders in place of `{}`. A function `fill(template, values)` in `app/widgets.rs` puts the values in, and every call site uses it in place of `format!`. Numbers are formatted before they are put in (`{:.1}` megabytes, zero-padded date parts), so a template holds words and placeholders only. The templates are: the window title (`app/mod.rs`), `"indexed {indexed}, removed {removed}, failed {failed}, in {seconds}s"`, `"Loaded {folder}"`, `"{label}  {milliseconds} ms"`, `"listing the folder: {count}"`, `"{count} files could not be read"` and `"{path}: {message}"` (`app/scan_page.rs`), `"{doing} {done} of {total}"`, `"{verb} {count} files"`, `"{megabytes} MB"`, `"{path}  {reason}"`, `"{count} dropped from the index"` and `"removed {removed} files, freed {megabytes} MB, {failed} failed, {index}"` (`app/cleanup_page.rs`), `"{width}x{height}  {format}  {megabytes} MB"`, `"{width}x{height}"` and `"{format}  {megabytes} MB"` (`app/review_page.rs`), `"{count} to remove"`, `"{megabytes} MB to reclaim"`, `"{label} {percent}%"` and `"{count} {noun}"` (`app/widgets.rs`), and `"{year}-{month}-{day} {hour}:{minute}"` (`app/dates.rs`). Log line templates stay where they are. A test in `tests/app.rs` fills every template and checks no placeholder is left in the result.
- [x] 10. Template docs: update anything in `docs/` that the changes from step 9 make out of date, and describe `template_strings.rs` and `fill` where the docs describe `constants.rs`.
